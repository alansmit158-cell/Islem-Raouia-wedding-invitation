...
const TimelineItem = ({ time, title, description, mapLink, icon: Icon, isLast, language }) => (
    <div className="flex gap-4 sm:gap-8 relative pb-12 last:pb-0 group">
        ...
        <div className="pt-1 sm:pt-2 text-left flex-1">
            ...
            <div className="flex items-start justify-between gap-4">
                <div>
                    <h4 className="font-script text-2xl sm:text-3xl text-wedding-blue-roi">{title[language] || title}</h4>
                    {description && <p className="text-gray-500 text-sm mt-1 leading-relaxed">{description[language] || description}</p>}
                </div>
                {mapLink && (
                    <a
                        href={mapLink}
                        ...
                        title={language === 'it' ? 'Vedi su Google Maps' : 'Voir sur Google Maps'}
                    >
...
export default function Timeline({ weddingData, language = 'fr' }) {
    return (
